	...
	tests := []test.Test{
		{
			Test: func(db *gorm.DB) {
				...
				surveyObj, err := query.GetSurvey(db, userOne.UserId, survey.Generic_v1.Group)
				...
				err = query.SaveSurveyResponses(db, userOne.UserId, survey.Generic_v1.Group, survey.Generic_v1.Version, responses)
				assert.NoError(t, err)
				surveyObj, err = query.GetSurvey(db, userOne.UserId, survey.Generic_v1.Group)
	...
	tests := []test.Test{
		{
			...
			Test: func(db *gorm.DB) {
				...
				genericResponse, err := query.GetSurveyResponses(db, userId, survey.Generic_v1.Group)
				testResponse, err := query.GetSurveyResponses(db, userId, testSurvey.Group)
				// Assert no responses initially.
				...
				err = query.SaveSurveyResponses(db, userId, survey.Generic_v1.Group, survey.Generic_v1.Version, responses)
				assert.NoError(t, err)
				responses = data.SurveyResponses{testSurvey.Questions[0].Key: testSurvey.Questions[0].Options[0].Key}
				err = query.SaveSurveyResponses(db, userId, testSurvey.Group, testSurvey.Version, responses)
				assert.NoError(t, err)
				genericResponse, err = query.GetSurveyResponses(db, userId, survey.Generic_v1.Group)
				assert.NoError(t, err)
				testResponse, err = query.GetSurveyResponses(db, userId, testSurvey.Group)
		...
		},
		{
			...
			Test: func(db *gorm.DB) {
				userId := data.TUserID(1)
				survey, err := query.GetSurvey(db, userId, data.SurveyGroup("bad_survey_group"))
				assert.Nil(t, survey)
